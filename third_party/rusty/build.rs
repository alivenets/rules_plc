...
fn locate_llvm_config() -> Option<PathBuf> {
    ...
    // In Bazel, llvm-config may be reachable from either execroot-style or runfiles-style paths.
    // Probe both layouts to avoid depending on a single working-directory assumption.
    for candidate in [
        PathBuf::from("external/toolchains_llvm++llvm+llvm_toolchain_llvm/bin"),
        PathBuf::from("toolchains_llvm++llvm+llvm_toolchain_llvm/bin"),
    ] {
        if let Some(x) = llvm_compatible_binary_name(&candidate) {
            return Some(x);
        }
    }
    ...
    if let Ok(manifest_dir) = env::var("CARGO_MANIFEST_DIR") {
        ...
        for candidate in [
            manifest_dir.join("../../toolchains_llvm++llvm+llvm_toolchain_llvm/bin"),
            manifest_dir.join("../../../toolchains_llvm++llvm+llvm_toolchain_llvm/bin"),
            manifest_dir.join("../../../../external/toolchains_llvm++llvm+llvm_toolchain_llvm/bin"),
        ] {
            if let Some(x) = llvm_compatible_binary_name(&candidate) {
                return Some(x);
            }
        ...
        .map(|flag| {
            if target_env_is("msvc") {
                ...
                if maybe_lib.is_file() {
                    ...
                    if let Some(soname) = Path::new(flag).file_name().and_then(|n| n.to_str()) {
                        if let Some(name) = [LibraryKind::Dynamic, LibraryKind::Static]
                            .iter()
                            .filter_map(|&kind| {
                                if let Some((stem, _rest)) = soname.rsplit_once(kind.file_extension()) {
...
fn get_system_library_dirs() -> impl IntoIterator<Item = String> {
    ...
    if let Some(dirs) = option_env!("LLVM_SYS_SYSTEM_LIBRARY_DIRS") {
        println!("cargo:warning=LLVM_SYS_SYSTEM_LIBRARY_DIRS={dirs}");
...
impl LinkingPreferences {
    fn init() -> LinkingPreferences {
        ...
        // If no preference is given, prefer dynamic linking for Bazel integration
        // to avoid missing C API symbols from component-wise static archives.
        let prefer_static = if prefer_static || force_static {
            true
        } else if prefer_dynamic || force_dynamic {
            false
        } else {
            false
        };
...
fn main() {
    ...
    if !cfg!(feature = "disable-alltargets-init") && Path::new("wrappers/target.c").exists() {
